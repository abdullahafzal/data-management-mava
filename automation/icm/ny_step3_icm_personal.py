...
Flow per owner (file order):
  ...
  5. Save to master:
       icm_verified = correct | partial | incorrect | no_results | error (quality flag)
       icm_phone_numbers, icm_emails, icm_locations_full, icm_page_url, etc. (all scraped data)
       personal_cell / personal_email = first phone/email found (always saved when ICM returns them)
...
def _empty_icm_fields(*, verified: str = "no_results", status: str = "no_results") -> dict[str, str]:
    return {
        "icm_verified": verified,
        "icm_name_match": "no",
...
def _build_icm_master_update(
    records: List[icm.NameSearchResultRow],
    row: dict[str, Any],
) -> dict[str, str]:
    """
    Build all master columns: verification flags plus every phone, email, location,
    and report URL scraped from Instant Checkmate.
    """
    if not records:
        return _empty_icm_fields()
    ...
    for rec in records:
        ...
        if v["icm_location_match"] == "yes":
            rank += 2
        if (rec.phone_numbers or "").strip() or (rec.emails or "").strip():
            rank += 1
    ...
    for rec in records:
        all_phones.extend(_split_pipe(rec.phone_numbers))
        all_emails.extend(_split_pipe(rec.emails))
        loc_src = (rec.locations or rec.result_card_locations or "").strip()
        all_locs.extend(_split_pipe(loc_src))
        if rec.page_url:
            page_urls.append(str(rec.page_url).strip())
        if rec.error:
            errors.append(str(rec.error).strip())

    phones = _dedupe_ordered(all_phones)
    emails = _dedupe_ordered([e.lower() for e in all_emails])
    locs = _dedupe_ordered(all_locs)

    out = dict(best_v)
    out["icm_status"] = (best_rec.status or best_v.get("icm_status") or "").strip()
    out["icm_phone_numbers"] = " | ".join(phones)
    out["icm_emails"] = " | ".join(emails)
    out["icm_locations_full"] = " | ".join(locs)
    out["icm_page_url"] = page_urls[0] if page_urls else (best_rec.page_url or "")
    out["icm_result_card_name"] = (best_rec.result_card_name or "").strip()
    out["icm_card_locations"] = (best_rec.result_card_locations or "").strip()
    out["icm_records_found"] = str(len(records))
    out["icm_scrape_error"] = " | ".join(_dedupe_ordered(errors))[:500]

    report_name = (best_rec.report_name or best_rec.result_card_name or "").strip()
    if report_name and not out.get("icm_report_name"):
        out["icm_report_name"] = report_name
    if locs and not out.get("icm_locations_found"):
        out["icm_locations_found"] = " | ".join(locs[:3])

    out["personal_cell"] = phones[0] if phones else ""
    out["personal_email"] = emails[0] if emails else ""
...
def _pick_verified_contact(
    ...
    """Legacy helper — prefer _build_icm_master_update for full scrape + verify."""
    fields = _build_icm_master_update(records, row)
    verified = {k: fields[k] for k in (
        "icm_verified", "icm_name_match", "icm_location_match",
        "icm_report_name", "icm_locations_found", "icm_status",
    ) if k in fields}
    return verified, fields.get("personal_cell", ""), fields.get("personal_email", "")


def master_rows_to_icm_input(
    master: pd.DataFrame,
    column_map: dict[str, str] | None = None,
) -> list[dict[str, Any]]:
    from automation.services.names import split_full_name, to_search_parts

    cmap = nd._norm_col_map(master)

    if column_map:
        name_mode = column_map.get('name_mode', 'full_name')
        c_state = column_map.get('state')
        c_city = column_map.get('city')
        c_street = column_map.get('address')
        c_age = column_map.get('age')
        c_own_ov = column_map.get('owner_overflow')
        c_num = cmap.get('facility #') or cmap.get('facility number')
        c_fname = cmap.get('facility name')

        if not c_state or c_state not in master.columns:
            raise ValueError('State column mapping is missing or not in file.')
        if not ((c_city and c_city in master.columns) or (c_street and c_street in master.columns)):
            raise ValueError('City or address column mapping is required.')

        if name_mode == 'full_name':
            c_owner = column_map.get('full_name')
            if not c_owner or c_owner not in master.columns:
                raise ValueError('Full name column mapping is missing or not in file.')
        else:
            for key in ('first_name', 'last_name'):
                col = column_map.get(key)
                if not col or col not in master.columns:
                    raise ValueError(f'{key.replace("_", " ").title()} column mapping is missing.')
            c_owner = None
    else:
    ...
    for i, ser in master.iterrows():
        if column_map and name_mode == 'full_name':
            owner = nd._cell_str(ser.get(column_map['full_name']))
        elif column_map:
            first_raw = nd._cell_str(ser.get(column_map['first_name']))
            last_raw = nd._cell_str(ser.get(column_map['last_name']))
            mid_col = column_map.get('middle_name') or ''
            mid_raw = nd._cell_str(ser.get(mid_col)) if mid_col else ''
            owner = ' '.join(p for p in [first_raw, mid_raw, last_raw] if p).strip()
        else:
        ...
        if column_map:
            own_ov = nd._cell_str(ser.get(c_own_ov)) if c_own_ov and c_own_ov in master.columns else ""
        else:
        ...
        if column_map:
            fc = nd._cell_str(ser.get(c_city)) if c_city and c_city in master.columns else ""
            fs = nd._cell_str(ser.get(c_state)) if c_state else ""
            fst = nd._cell_str(ser.get(c_street)) if c_street and c_street in master.columns else ""
            age_val = nd._cell_str(ser.get(c_age)) if c_age and c_age in master.columns else ""
        else:
        ...
        first, last, mid = "", "", ""
        if otype == "individual":
            if column_map and name_mode == 'split_columns':
                first = nd._cell_str(ser.get(column_map['first_name']))
                last = nd._cell_str(ser.get(column_map['last_name']))
                mid_col = column_map.get('middle_name') or ''
                mid = nd._cell_str(ser.get(mid_col)) if mid_col else ''
                parts = to_search_parts(first, last, mid)
                first, last, mid = parts['search_first_name'], parts['search_last_name'], parts['search_middle']
            elif column_map:
...
def apply_icm_result_for_owner(
    ...
    for i, ser in master.iterrows():
        ...
        fields = _build_icm_master_update(recs, row)
        for k, v in fields.items():
            master.at[i, k] = v

        owner = row.get("owner_name_raw", "")
        rep = fields.get("icm_report_name", "")
        flag = fields.get("icm_verified", "")
        n_phones = len(_split_pipe(fields.get("icm_phone_numbers", "")))
        n_emails = len(_split_pipe(fields.get("icm_emails", "")))
        print(
            f"  excel~{prid} | NY: {owner!r} | ICM: {rep!r} | "
            f"phones={n_phones} emails={n_emails} | "
            f"name={fields.get('icm_name_match')} loc={fields.get('icm_location_match')} "
            f"→ {flag.upper()}"
        )
        break
    return master
...
def apply_icm_results_to_master(
    ...
    for i, ser in master.iterrows():
        ...
        fields = _build_icm_master_update(recs, src)
        ...
        print(
            f"  excel~{prid} | NY: {owner!r} | ICM: {rep!r} | "
            f"phones={fields.get('icm_phone_numbers', '')!r} | "
            f"name={fields.get('icm_name_match')} loc={fields.get('icm_location_match')} "
            f"→ {flag.upper()}"
        )
        updated += 1

    print(f"[Step3] Updated {updated} master row(s) with ICM data.")
    return master
